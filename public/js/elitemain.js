...
DIB.showMessage = function (message, title, reloadOnClose, redirectOnClose, width, divId)
{
    ...
    dialogElement.dialog({
        ...
        buttons: {
            OK: {
                class: "btn waves-effect waves-light btn-rounded btn-success primary",
                text:"OK",
                click: function () {
...
{
    var date_issue =new Date($(date_issue_field).val());
    var today = new Date();
...
FORM.validateFile = function (input, maxSize, maxSizeExeededMessage) {
    ...
    for (var i = 0; i < input[0].files.length; i++) {
        ...
        if (input[0].files[i].size / 1000000 > maxSize) {
           ...
           // DIB.displayErrors(maxSizeExeededMessage, LOCALE.get('insly.common.whoops'));
            DIB.alert(maxSizeExeededMessage, LOCALE.get('insly.common.whoops'));
            flag = false;
            if (!flag) {
                return false;
            }
...
POLICY.setEndDate = function (months, policy_duration_adjustment, date_start_field, date_end_field)
{
    ...
    var endDate = new Date($(date_start_field).val());
    endDate.setMonth(endDate.getMonth() + months);
    endDate.setDate(endDate.getDate() + policy_duration_adjustment-1);

    $(date_end_field).val(moment(endDate).format('YYYY-MM-DD'));
}
